...
export function createSchoolUI({api,toast}){
 ...
 function loginCard(){return `<section class="card span" id="school-account"><div class="card-head"><h2 class="icon-heading tone-info">${pixelIcon('i-key','heading-icon')}研究生教务登录</h2>${unverifiedBadge()}</div><p class="muted">使用研究生选课系统的学号和密码。校园网登录与这里分开；不保存密码、不自动选课或退课。</p><div id="school-status">${status()}</div><details id="school-login-details" ${logged?'':'open'}><summary>${logged?'切换账号 / 重新登录':'登录研究生教务'}</summary><form id="school-login-form" autocomplete="off"><div class="grid"><div><label for="school-account-input">学号</label><input id="school-account-input" name="username" autocomplete="off" maxlength="80" required placeholder="输入学号，默认不回填"></div><div><label for="school-password">教务密码</label><input id="school-password" name="password" type="password" autocomplete="new-password" maxlength="128" required placeholder="仅用于本次登录"></div></div><div id="school-captcha">${captchaHTML()}</div><div class="actions"><button type="submit" class="primary" ${!challenge||busy?'disabled':''}>${pixelIcon('i-key')}登录教务</button>${action('获取 / 更换验证码','challenge','type="button"')}</div></form></details><div class="actions">${action('清除本次登录','clear',logged?'':'disabled')}<a class="button quiet" href="${official}" target="_blank" rel="noopener noreferrer">学校原页面 ↗</a></div></section>`}
 function timetableCard(){return `<section class="card span" id="school-timetable-card"><div class="card-head"><h2 class="icon-heading tone-info">${pixelIcon('i-book','heading-icon')}我的课表</h2>${unverifiedBadge()}</div><p class="muted">课表从学校系统读取，只在本次运行中显示、不写入庭院备份；调课、考试与历史学期请以官方系统为准。</p><h3>研究生 · 本学期课表</h3><div class="actions">${action('读取我的课表','read',logged?'':'disabled')}</div><p class="muted" id="school-timetable-hint">${logged?'已登录研究生教务，可以直接读取。':'需要先在上面的「研究生教务登录」里登录；读取的是当前选课学期。'}</p><div id="school-timetable" aria-live="polite">${timetableHTML(data)}</div><details><summary>本科 · 我的课表 ${unverifiedBadge()}</summary><p class="muted">本科使用统一身份认证。先打开「本科我的课表」完成学校登录，再在下方成绩区域保存该业务页面的 ehall Cookie。本科权限单独核验；研究生登录不能代替它。</p><div class="actions"><a class="button quiet" href="https://ehall.szu.edu.cn/jwapp/sys/wdkb/*default/index.do" target="_blank" rel="noopener noreferrer">本科我的课表 ↗</a></div><label for="undergrad-term">学期（留空则向学校查询当前学期）</label><input id="undergrad-term" maxlength="11" placeholder="例如 2026-2027-1"><div class="actions">${action('读取本科个人课表','undergrad')}</div><div id="undergrad-timetable" aria-live="polite">${undergradHTML()}</div><p class="muted">根据个人课表接口接入，尚待有本科权限的账号实测。保留学校返回的时间地点原文，暂不拆成每周网格。</p></details></section>`}
 ...
 return {loginCard,timetableCard,load,click,submit,sync:paint};
}
