...
export function createAcademicUI({getState,api,toast}){
 ...
 function summary(){
  ...
  return `<div class="card-head"><h2 class="icon-heading tone-success">${pixelIcon('i-calendar','heading-icon')}本学期</h2><span class="badge" data-tone="${current.manual?'warning':'success'}">${current.manual?'手动日期':'官方校历'}</span></div><p class="academic-week">${!calendar&&!current.manual?'正在读取校历…':esc(current.title)}</p><p class="muted">${esc(current.detail)}</p>${current.term?`<p class="academic-dates">学期 ${esc(current.term.start)} — ${esc(current.term.end)}<br>第 1 周从 ${esc(current.term.week_start)} 开始，周日换周。</p>`:''}<p class="notice" data-tone="${loading?'info':error||calendar?.stale?'warning':calendar?'success':'muted'}" role="status">${loading?'正在检查学校校历…':error?esc(error):calendar?.message?esc(calendar.message):calendar?`${calendar.stale?'上次校历 · ':''}${calendar.checked_at&&!calendar.checked_at.startsWith('0001')?'已检查 '+esc(new Date(calendar.checked_at).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',month:'numeric',day:'numeric',hour:'2-digit',minute:'2-digit'})):'等待联网检查'}；每天自动检查更新。`:'首次读取中'}</p>`;
 }
