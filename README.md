...
**beta0.2 已接入校外 VPN**：校园网判区、登录、注销、断线诊断和 VPN 操作页都接了真内核；校外真实网络验证仍待实机完成。
...
当前公开测试版为 `beta0.2`。本版本新增校外 VPN 页面和校内后端服务入口预留。发布前必须依次通过全仓测试、页面静态检查、真实点击验收和 Windows 整机冒烟，再生成 ZIP：
