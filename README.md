...
**尚未发布的学院公告筛选**：选择学院后自动读取公开栏目；计软等暂未接入的院系提供官网入口。不包含需登录的校内公告，公开 beta0.6 下载包尚未包含此更新。

**尚未发布的预约体验修复**：当前源码撤下了预约 Cookie 输入、F12 教程和未经真实验收的本地提交表单。空位为只读预览，点击「登录并预约」在浏览器打开学校页面。公开 beta0.6 下载包尚未包含这项调整；原网页内嵌与真实登录流程仍待验证。
...
官方校历与教学周：启动后读取本机缓存，每天检查学校校历页；Windows 在校历图片更新时调用本机系统文字识别，失败保留旧日期并提示，可手动覆盖。研究生提供本地登录与课表读取；本科通过「我的课表」业务 Cookie 读取时间地点原文，两者仍待真实账号完整验收。社区场地可在校园网内读取公开场地和实时空位；当前源码通过「登录并预约」进入学校原页，由用户完成登录、提交并查看结果。已发布 beta0.6 中的预约会话输入与实验性本地提交入口已在当前源码撤下，下载包尚未更新。当前验收状态统一见 [STATUS.md](docs/STATUS.md)。
