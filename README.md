...
当前已发布版本仍为 **[szuDesktop-Setup-0.9.0.exe](https://github.com/SzuDesktopTeam/szudesktop/releases/download/beta0.9.0/szuDesktop-Setup-0.9.0.exe)**，运行安装程序后打开 szuDesktop。安装版自带窗口运行时，无需额外安装浏览器或开发环境。便携版、命令行和校验文件见 [beta0.9.0 发布页](https://github.com/SzuDesktopTeam/szudesktop/releases/tag/beta0.9.0)。本分支正在准备 beta0.9.1，尚未提供新版下载。
...
| 当前源码版本 | `beta0.9.1` · 发布候选，实际外壳验收、PR 与发布检查待完成；公开附件仍为 beta0.9.0 |
...
2026-09-26 已发布：[PR #13](https://github.com/SzuDesktopTeam/szudesktop/pull/13) 已合并，[版本检查](https://github.com/SzuDesktopTeam/szudesktop/actions/runs/36220458700) 全部通过，含安装、主窗与宠物、重启尺寸恢复、同版本重装保留存档和卸载。后续真实账号复测已通过研究生备用登录及课表读取，其他业务的具体边界见 [STATUS 第 47 节](docs/STATUS.md#47-学校真实账号复测2026-09-26)。

**beta0.9.1 发布候选，尚未进入下载包**：宠物新增左键/右键菜单、滚轮调节大小、拖动与位置记忆，照料操作保存到庭院存档。界面保留像素深大和木质导航，使用浅纸色、绿色主按钮及更清晰的文字层次；首页缩短欢迎区，去掉重复的网络状态，增加伙伴场景卡片，并统一表单、服务入口与窄屏布局。另包含学校认证跳转修复。具体修改与验收统一记录在 [STATUS](docs/STATUS.md)。

### 桌面伙伴（beta0.9.1 候选安装版）
...
- 安装版关闭主窗口后保留宠物与托盘；beta0.9.1 候选版从宠物菜单选择「打开主窗口」，也可点击托盘重开。通过宠物菜单「退出应用」、托盘「退出」或「设置 → 退出应用」完整退出；只清理本次启动的引擎，复用的已有后台服务保留。便携版仍在所有窗口关闭约 10 秒后退出
- 刷新页面不会结束服务
...
以下说明当前源码能力；beta0.9.1 仍为发布候选，公开下载是 beta0.9.0。已验证的查询与待验收的账号业务分别标明，详细状态统一见 STATUS.md。
...
| 桌面宠物（安装版） | 发布候选 | 独立透明宠物窗、左/右键菜单和照料、40%–200% 滚轮/滑杆缩放、拖动、重启恢复大小与位置；本轮实际外壳和安装包验收待完成 |
...
**当前限制**：成绩仍只读第一页，余额未接入。研究生备用登录及当前课表已通过本轮真实读取；本科入口对当前测试账号返回 403，研究生成绩原页未加载出列表，学院琴房连接失败。官方浏览器已完成预约时段选择并打开确认表单，未提交预约；候选安装版的详情仍未加载，后续浏览器刷新也出现连接中断。完整预约与业务会话接回仍待完成，不能据此宣称全部业务验收通过。
...
本轮研究生备用登录已使用真实账号通过，并在应用中显示学校返回的当前学期与待安排课程；学校原页同样未返回排课安排。这个结果尚未覆盖非空排课、成绩或本科权限。beta0.9.1 候选版另修复了 WebVPN 认证跳转被拦截及登录提示指向旧入口的问题，尚未包含在上方 beta0.9.0 下载包中。
...
安装版关主窗口后继续常驻；beta0.9.1 候选版从宠物菜单选择「打开主窗口」，或点击托盘重新打开。从宠物菜单「退出应用」或托盘「退出」完整退出并清理自己启动的引擎，复用的已有服务保留。便携版关闭**所有**窗口约 10 秒后退出；
