...
当前版本为 **[szuDesktop-Setup-0.9.1.exe](https://github.com/SzuDesktopTeam/szudesktop/releases/download/beta0.9.1/szuDesktop-Setup-0.9.1.exe)**，运行安装程序后打开 szuDesktop。安装版自带窗口运行时，无需额外安装浏览器或开发环境。便携版、命令行和校验文件见 [beta0.9.1 发布页](https://github.com/SzuDesktopTeam/szudesktop/releases/tag/beta0.9.1)。

| 项 | 说明 |
| :-- | :---- |
| Windows 安装版 | Electron 独立窗口，内含 Go 引擎；同名 `.sha256` 文件可用于校验下载 |
| Windows 便携版 | `szudesktop-beta0.9.1-windows-amd64.zip`，解压后双击 `szudesktop.exe`，使用本机 Edge / Chrome 开窗 |
| 命令行 | Windows / macOS / Linux 单文件 `szunet`，继续提供 |
| 当前版本 | `beta0.9.1` · 已发布，桌宠实际操作与安装包检查通过 |
| 存档 | 两种 Windows 版本使用同一份本机庭院与学习记录，更新前可在设置中导出备份 |

安装版提供常驻桌面宠物、系统托盘和 40%–200% 大小调节。关主窗口后宠物继续陪伴，可从宠物菜单选择「打开主窗口」，或点击托盘重开。学校登录和预约原页面可在安装版内打开，不再要求普通用户复制 Cookie。校历识别优先使用中文语言包。真实学校账号业务的验收边界见 [STATUS](docs/STATUS.md)。安装版开机自启仍未实现。

2026-09-26 发布：[PR #15](https://github.com/SzuDesktopTeam/szudesktop/pull/15) 已合并，[beta0.9.1 发布检查](https://github.com/SzuDesktopTeam/szudesktop/actions/runs/36233240617) 通过。实际外壳已验证菜单、隐藏主窗后的照料与库存变化、农田和学习导航、缩放、拖动及重启恢复；Windows 安装流程也已通过。研究生备用登录及当前课表读取已有真实证据，其他学校业务边界见 [STATUS 第 47 节](docs/STATUS.md#47-学校真实账号复测2026-09-26)。

**beta0.9.1 更新**：宠物新增左键/右键菜单、滚轮调节大小、拖动与位置记忆，照料操作保存到庭院存档。界面保留像素深大和木质导航，使用浅纸色、绿色主按钮及更清晰的文字层次；首页缩短欢迎区，去掉重复的网络状态，增加伙伴场景卡片，并统一表单、服务入口与窄屏布局。新版同时包含学校认证跳转修复。具体修改与验收统一记录在 [STATUS](docs/STATUS.md)。
...
本地开发版将基础伙伴扩为 **荔宝、栗栗、小白（白鹭）、阿青（小龟）** 四位，增加庭院可视卡片选择和桌宠菜单「切换伙伴」，选择同步到桌面。旧存档只追加缺失的基础伙伴，保留当前选择、名字和成长。本地规则检查、真实 Electron + Go 窗口中的双向切换、隐藏主窗切换、重启恢复及 420px 窄窗检查已通过。**上方公开下载仍是 beta0.9.1，包含原有两位伙伴，这些新增内容尚未发布。**

![四伙伴选择卡片（开发版，尚未发布）](docs/screenshot-companions.png)

本轮同时推进成绩分页、学校退出登录后的旧会话清理，以及从 beta0.9.1 升级并保留数据的安装检查。备份验收覆盖真实窗口中的导出、取消恢复与确认恢复；首轮 CI 的数据保留检查通过，恢复脚本等待界面就绪的时序已修正，整条安装检查仍待重跑通过。检查结果统一记录在 [STATUS](docs/STATUS.md)。

1.0 先完成桌面版，**暂不部署校内后端或 Docker**。剩余任务包括校园网现场认证闭环、安装版学校登录与会话接回、本科/研究生真实课表与完整成绩、应用内空间预约、学院琴房权限验收，以及四伙伴发行、跨版本升级与最终交付。已有验收证据继续有效，具体缺口和完成标准统一维护在 [STATUS 第 50.2 节](docs/STATUS.md#502-10-剩余任务暂不部署后端)。后端、云同步和自动更新延期；当前安装包未签名，跨版本升级尚未实测。
...
以下说明 beta0.9.1 的实际能力。已验证的桌面功能与查询、待验收的学校账号业务分别标明，详细状态统一见 STATUS.md。
...
| 学习 · 成绩与绩点 | 部分 | 支持粘贴 / CSV / TSV 导入及本机绩点计算；学校登录后可手动查询成绩，仍待真实账号验收，仅查第一页，不自动并入绩点。**不解析 PDF / 图片 / XLSX** |
...
**当前限制**：成绩仍只读第一页，余额未接入。研究生备用登录及当前课表已通过本轮真实读取；本科入口对当前测试账号返回 403，研究生成绩原页未加载出列表，学院琴房连接失败。官方浏览器已完成预约时段选择并打开确认表单，未提交预约；安装版的详情仍未加载，后续浏览器刷新也出现连接中断。完整预约与业务会话接回仍待完成，不能据此宣称全部业务验收通过。
...
便携版在「备用登录」中保留账号密码和手动会话入口。备用会话保存使用系统加密设施，失败时拒绝保存；不要把密码或 Cookie 发到聊天或反馈中。在线成绩目前只读第一页，不自动并入本机绩点统计；总数未知或未取全时会标明。本科保留学校时间地点原文，研究生展示学校返回的课程安排。
