...
当前源码增加了本科 / 研究生在线成绩读取，尚未完成真实成绩验收，也尚未更新到现有 Release 附件。用户仅在本机应用里输入对应成绩业务的 Cookie；系统安全存储不可用时拒绝保存。验证按培养层次分别进行，无权限与会话失效分开提示；目前只查询第一页，总数未确认或尚未取全时会明确标示。社区静音舱与体育场馆仍需独立接入，不能由成绩验证推断可用。
