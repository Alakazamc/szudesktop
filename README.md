...
当前开发分支还加入了官方校历与教学周：启动后读取本机缓存，每天检查学校校历页；Windows 在校历图片更新时调用本机系统文字识别，失败保留旧日期并提示。可手动覆盖，也可恢复自动。学习页新增本科 / 研究生官方课表入口，应用内课表同步与预约接入仍未完成。以上改动尚未进入已发布的 beta0.5.1 安装包。
