...
| 当前版本 | [beta0.7.2](https://github.com/SzuDesktopTeam/szudesktop/releases/tag/beta0.7.2) · 公开测试版（预发布），Windows 下载包已发布 |
...
beta0.7.2 为公开测试版。已验证的查询与待验收的账号业务分别标明，详细状态统一见 STATUS.md。
...
macOS 上保存凭据（`config set`）走系统钥匙串：写入前会先用一次性条目自检这条路可用，失败时明确报错，不会退回把密码写进命令行参数（那样同机其他进程能看到）。**边界如实说明**：这条路径还没有在真实 macOS 上验收过；CI 已加一个 `macos-latest` job，用一次性钥匙串直接对真实 `security` 命令验证「`-w` 放末尾会退回读标准输入」这个假设，但该 job 尚未运行过、也不阻断发布（F25）。如果自检假设不成立，`config set` 会**明确报错**而不是静默泄漏密码——但也就意味着你可能存不了凭据。跟踪记录见 STATUS.md 的 F21 / F25。
