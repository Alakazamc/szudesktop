...
**当前限制**：成绩仍只读第一页，余额未接入。研究生备用登录及当前课表已通过本轮真实读取；本科入口对当前测试账号返回 403，研究生成绩原页未加载出列表，学院琴房连接失败。候选安装版已完成 WebVPN 验证并打开预约列表，但详情的房间信息和时段仍未加载；完整预约与业务会话接回仍待完成，不能据此宣称全部业务验收通过。
