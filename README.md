...
推荐下载 **[szuDesktop-Setup-0.9.0.exe](https://github.com/SzuDesktopTeam/szudesktop/releases/download/beta0.9.0/szuDesktop-Setup-0.9.0.exe)**，运行安装程序后打开 szuDesktop。安装版自带窗口运行时，无需额外安装浏览器或开发环境。便携版、命令行和校验文件见 [beta0.9.0 发布页](https://github.com/SzuDesktopTeam/szudesktop/releases/tag/beta0.9.0)。
...
**开发分支界面更新，尚未进入下载包**：保留像素深大背景和木质导航，使用更浅的纸色、绿色主按钮及更清晰的文字层次。首页缩短欢迎区，去掉重复的网络状态，增加伙伴场景卡片，并统一表单、服务入口与窄屏布局。具体修改和验收仍统一记录在 [STATUS](docs/STATUS.md)。
...
本轮研究生备用登录已使用真实账号通过，并在应用中显示学校返回的当前学期与待安排课程；学校原页同样未返回排课安排。这个结果尚未覆盖非空排课、成绩或本科权限。开发分支另修复了 WebVPN 认证跳转被拦截及登录提示指向旧入口的问题，尚未包含在上方 beta0.9.0 下载包中。
