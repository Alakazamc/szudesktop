...
**一个必须说明的缺陷（F26）**：CI 新加的 macOS 探针在真机上跑出 `passwords don't match`——
真实的 `security -w` 会**问两遍**（密码 + 确认），而 beta0.7.1 与 beta0.7.2 只喂了一行，
所以那两个版本的 macOS 命令行版**根本存不了凭据**（会明确报错，不泄漏、不落明文，但功能不可用）。
现已改为喂「密码 + 确认」两行，**修复尚未发布**；那条探针也不再允许失败被忽略，
并且已成为发布的前置条件。跟踪记录见 STATUS.md 的 F21 / F25 / F26。
