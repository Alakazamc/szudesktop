...
安装版修复了已有后台引擎的复用、启动超时和退出清理，并升级至 Electron 44.4.5。启动只检查本机引擎，校园网或外网不可达也能打开庭院；网络状态在后台查询。公开查询现在只在对应卡片等待，查询时可以切换页面。完整验证记录见 [STATUS](docs/STATUS.md)，CI 安装测试保留页面截图及失败原因。常驻宠物窗、托盘与安装版开机自启仍属后续阶段。
