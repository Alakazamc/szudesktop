...
> 更新：2026-09-26。合并原 STATUS、任务清单、界面重设计、本地任务副本及两轮代码/视觉审查。
> 原独立清单和设计方案已移除；本地原始调研只作为冻结的历史证据，不再维护另一套任务。
> 本文是公开安全版本，不包含真实账号、内网拓扑或未公开接口细节；旧调研中的数量、规则、可达性和许可结论须在实际接入时重核。

## 1. 当前状态

- 已发布 **[beta0.9.0](https://github.com/SzuDesktopTeam/szudesktop/releases/tag/beta0.9.0)**，PR #13 已合并（`9d82a57`）；标签 CI 36220458700 全部通过，10 个发行附件已公开。Windows Electron 安装、重开、宠物/托盘、尺寸恢复、便携引擎共存、同版本重装和卸载通过，见第 46 节。
- 当前源码已升为 **beta0.9.1 发布候选**：整合像素校园界面打磨、学校认证跳转修复，以及宠物点击菜单、滚轮缩放、拖动记忆和存档照料。本轮实际外壳验收、PR 检查、安装包与标签发布待完成；尚未进入公开下载，见第 49 节。
- beta0.9.0 整合校历 OCR、桌面宠物与尺寸/动作、学院琴房和学校登录分支，新增安装版学校原页窗口与内存会话接回。PR #10、#11、#12 的代码已由 #13 统一交付，三个旧 PR 已关闭。
- 安装版关主窗口后宠物与托盘继续运行；从宠物或托盘重开，完整退出使用托盘或设置。便携版保持所有窗口关闭约 10 秒退出。
- 真实账号复测已有部分通过：研究生学校原页登录、应用备用登录与当前课表读取成功，能区分无排课和非空已选课程。安装版 WebVPN 认证跳转缺陷已在开发分支修复，真实窗口到达二次验证。本科、成绩、安装版会话接回、完整预约及琴房权限仍未验收通过；见第 47 节。
- 当前待办只维护在**第 32 节**；历史阶段记录不代表当前完成状态。README 中英两版随每次更新同步。
...
| 3 | P1 / S–M | 常驻宠物、托盘、大小/动作 | beta0.9.0 基础功能已发布。beta0.9.1 候选增加点击菜单、真实存档照料、滚轮缩放、拖动与重启恢复位置；本轮实际外壳与最终包检查待完成。物理多显示器待设备 |
| 4 | P1 / L | 学校原页面在应用内登录与预约 | 已修复 WebVPN 认证域名漏放行；本人已完成二次验证。对照浏览器能选时段并打开确认表单，未提交；原生详情为空且连接仍不稳定，完整流程及会话接回未通过。修复纳入 beta0.9.1 候选，尚未发布 |
| 5 | P1 / L | 本科/研究生课表与成绩 | 研究生备用登录和真实课表读取通过，空排课与待安排课程正确展示；尚未覆盖非空排课和安装版会话接回。本科原页对当前账号返回 403，研究生成绩原页未显示列表，需分别继续核验。成绩仍只查第一页 |
| 6 | P2 / M | 学院琴房只读查询 | 已整合 #11，密码用后清空、分页、切换失败清旧登录；本轮连接在登录前被关闭，不能据此判定账号权限。仍需可达网络及该系统授权账号，不同于社区共享琴房 |
| 7 | P1 / M | beta0.9.1 安装包交付 | 发布候选；待实际宠物操作、PR 全部检查、合并、标签流水线及 10 个发行附件核对。beta0.9.0 已交付的历史证据见第 46 节 |
| 8 | P2 / M | 其余 11 个学院公告栏目 | 仍为官方入口，逐个核实公开 HTML 后适配 |
| 9 | P2 / L | 图书馆选座、余额/流量、成绩取全与并入统计 | 未实现；需要官方接口与权限证据，不能套用另一系统登录成功 |
| 10 | P2 / M | 安装版自启、宠物通知与常驻偏好 | 后续工作；当前只支持手动开启，尺寸记忆已做，宠物隐藏状态尚不跨重启记忆 |
| 11 | P3 / L | 校内公共后端、校历假期/考试数据、场地照片 | 未部署/未获完整官方来源；不编造数据 |
...
上述修改尚未发布，不属于 beta0.9.0 已下载附件。候选外壳已构建并实际启动；域名策略回归与 7 项课表前端检查通过。没有修改系统代理、证书验证或学校安全策略。
...
验证：编译并启动隔离配置的真实 Go 程序，用实际浏览器检查首页上下区域、桌面服务与学习页、390px 窄屏六个页面，以及暮色庭院；未发现横向溢出或页面控制台错误。现有 UI 13 项、交互 9 项检查及语法检查通过。两份 README 同步更新。修改位于开发分支，**尚未发布新安装包**；本轮视觉检查不替代第 47 节的学校业务验收。

## 49. 宠物交互与 beta0.9.1 发布候选（2026-09-26）

本轮按用户要求补齐宠物的大小调节、点击菜单和常用功能，完成后再推送、合并 PR、发布版本。版本源已调整为 `beta0.9.1`；第 47 节学校认证修复与第 48 节界面改进一并进入候选。**本地真实外壳检查已通过，PR/标签检查和新版安装包发布仍待完成。**

| 事项 | 重要性 / 难度 | 候选实现与剩余验收 |
|---|---|---|
| 点击宠物只能打开主窗，功能入口不够直观 | P1 / S | 左键/右键打开菜单，显示伙伴状态；提供庭院、农田、学习、主窗口、隐藏和退出入口，待真实窗口逐项操作 |
| 大小调节藏在设置，无法直接在宠物上操作 | P1 / S | 增加滚轮每次 10 个百分点调节，保持 40%–200% 范围；菜单增减与默认大小、原设置滑杆和托盘预设共存，待边界与重启检查 |
| 宠物位置固定，重启后无法保留摆放 | P1 / M | 按住拖动、保存位置，与大小一同重启恢复；待实际拖动、点击区分、缩放后位置与重启检查，物理多显示器仍待设备 |
| 桌面照料与庭院存档需要一致 | P1 / M | 菜单摸头、喂食、陪玩和睡眠沿用庭院动作与本机存档，保留资源/冷却/精力规则与失败提示；新增 `check-pet-commands.mjs` 并纳入 CI，待真实持久化与反馈检查 |
| 新功能与下载说明同步交付 | P1 / M | 更新中英文 README、CHANGELOG 与便携包快速开始，明确 Electron 安装版与便携版区别；待 PR 全绿后合并、推标签，核对安装包/便携 ZIP 校验和及全部附件 |

发布检查继续保留 Go、界面、Electron 策略、macOS 凭据和 Windows 安装闭环；`smoke_installer.py` 仅在一次性 GitHub Windows runner 执行，不在开发机伪装 CI 运行。完成后在本节补实际检查、PR、标签与附件证据，不另建任务文档。

本地验证：Go 测试与静态检查通过；宠物命令 11 项、交互 9 项、庭院 13 项、存档界面 7 项、Electron 策略及 Windows 引擎冒烟通过，发布说明抽取 12 项通过。隔离配置中的真实 Electron + Go 已验证菜单、隐藏主窗后的睡眠/喂食、库存消耗、农田/学习导航、40%–200% 缩放、双轴拖动及位置恢复；另外实际点击检查了原生菜单和大小子菜单。打包版已完成首轮相同验收，拖动修正后的最终包继续通过发布流水线验收。证据留在忽略目录 `.scratch_probe/pet091-check` 和 `.scratch_probe/pet091-visual`，未将任何用户账号存档纳入发布。
