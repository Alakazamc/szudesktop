...
- 修复后的 Electron 候选窗口可访问学校 WebVPN 认证页，二维码与账号登录表单显示正常，账号密码被接受并进入学校二次验证。尚未把到达验证页记为预约登录完成。
...
| 学校预约全流程 | 原生窗口进入 WebVPN 二次验证，等待本人完成 | 进入场地详情、核对规则及提交入口；只有明确的真实场地和时段指令才创建预约 |
