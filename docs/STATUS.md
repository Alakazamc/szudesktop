...
> 更新：2026-09-25。合并原 STATUS、任务清单、界面重设计、本地任务副本及两轮代码/视觉审查。
...
- **当前待办总入口是第 32 节**；Electron 审查问题在第 44 节，修复与 beta0.8.0 安装包交付在第 45 节。
...
| 8 | **应用内承载学校原页（「套壳登录」，D1 / D2 / R04）** | 用户明确要的下一步：把成绩/课表的认证从「手工粘贴 Cookie」改成由应用自己承载学校原页完成统一身份认证。**现状 0% —— 没有任何 WebView 依赖、没有一行相关代码，本轮只登记不实施**，因为要先决定承载方案与它带来的三端分叉。详见 32.2 的第一条 | 待做（本轮只登记） |

### 32.2 代码侧 backlog

- **应用内承载学校原页（「套壳登录」，D1 / D2 / R04，2026-09-23 登记，完成度 0%）**：
  现在的三套认证机制互相独立，且没有一套是「应用内正常登录」——
  **成绩**（本科/研究生，`/api/scores?level=`）用用户**手工粘贴**的 ehall Cookie，落在系统安全存储；
  **本科课表**（`/api/academic/undergrad/timetable`）用**同一份粘贴的 Cookie**；
  **研究生课表**（`/api/academic/timetable`）由应用按学校协议自己登录（验证码 + DES 加密密码，**仅内存**）。
  （注：校园卡号只用于**校园网认证**，与学校业务无关，别和这三条混在一起。）
  想改成"应用内承载学校原页、由我们自己完成统一身份认证"，**目前一行都没有**：
  `go.mod` 里没有任何 webview 依赖（wails / zserge / WebView2 全无），
  当前窗口是 Edge / Chrome `--app` 模式（`desktop/internal/ui/server.go` 的 `openBrowser`），
  **不是原生嵌入式浏览器**，所以任何"套壳学校页面"的想法都要先新增依赖。
  已定约束（§15.1 / §21.2 / §26.3）：先做「粘贴 Cookie → 本机加密 → 复用 → 过期明确报错」的会话地基；
  **D2 已被降级**为后续「多一种获取方式」；**不会移除学校 iframe / CSP 防护强行嵌入**；
  **不以复制 Cookie 代替正常登录**；预约侧已撤下手工 Cookie 输入（R02 / R03 / R06），等正常登录衔接再恢复。
  一次 iframe 原型内嵌后持续空白，但**不能据此断言校方禁止嵌入**。
  落地前要先决定：承载方案（WebView2 / Wails，或继续用 `--app` 窗口）→ 三端（Windows / macOS / Linux）分叉 → 二次认证与结果的完整走查。
