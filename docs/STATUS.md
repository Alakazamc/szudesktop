...
| 3 | P1 / M | 常驻宠物、托盘、大小/动作 | 已随 beta0.9.0 发布；关窗重开、托盘显隐、滑杆、预设与重启恢复在真实 Electron 和最终安装包中通过。物理多显示器待设备 |
| 4 | P1 / L | 学校原页面在应用内登录与预约 | 独立学校窗口已实现；复测修复 WebVPN 认证域名漏放行，候选版已进入学校二次验证，等待本人完成。未提交预约，完整流程仍待验收；修复尚未发布 |
...
| 7 | P1 / M | beta0.9.0 安装包交付 | 已发布，10 个附件齐全；最终安装、重启、重装和卸载检查通过，安装包摘要与 CI 报告一致；见第 46 节 |
...
- 按此前已获同意的会议室使用须知进入详情，可见周日期、图例和“申请预约”按钮，但房间名称与可选时段未加载；切换到下一周仍无时段。未把空白界面当作无可用场地或预约全流程通过。另开官方浏览器对照页停在 WebVPN 二次验证，等待本人验证后对照。
...
| 学校预约全流程 | 原生窗口已完成 WebVPN 验证并进入列表、规则及详情；详情的房间信息、时段仍为空，尚未进入预约确认表单 | 对照浏览器完成验证后判断是官方业务登录/数据问题还是外壳兼容问题；只有明确的真实场地和时段指令才创建预约 |
